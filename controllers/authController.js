...
export const registerUser = async (req, res) => {
  try {
    ...
    // Send OTP - non-blocking, must run BEFORE res.status/return
    // (anything after return never executes)
    sendVerificationEmail(email, otp).catch((err) => {
      console.log("sendVerificationEmail failed:", err.message);
    });
...
export const resendUserOTP = async (req, res) => {
  try {
    ...
    // Non-blocking - agar email fail bhi ho, request crash na ho
    sendVerificationEmail(email, otp).catch((err) => {
      console.log("sendVerificationEmail failed:", err.message);
    });
